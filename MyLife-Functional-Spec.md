...
- **Last 7 Days**: shown only when the selected month + year match the current month + year. Data comes from a separate Firestore query (date range: today−7 through yesterday — never includes today) — completely independent of the monthly data, so cross-month days never affect the monthly count or summary. Boolean custom metrics show `X / 7` (always out of 7 regardless of how many records exist). Presentation differs by viewport:
  - **Desktop**: rendered as a **stat row at the top of the table, above the Goals row** (lavender tint, labeled "Last 7 Days"). Weight shows the net loss/gain over the window (colored green/red); Total Mi. shows the window's total miles (separate `exerciseActivities` range query, since the window can span the previous month); Sleep/Body Bat/Steps/Burn/Food/nutrition show averages; +/- Diff shows avg per day; exercise-type and Walk/Run/Dogs columns are blank. Not collapsible.
