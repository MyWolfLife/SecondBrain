...
- **📰 Recent news + 🤖 AI read (Phase 2, Stage 2.5)**: below the dips table, the dossier fetches the last 14 days of `anaFinnhubNews` (up to 10 dated, sourced, linked headlines; "News unavailable"/"No headlines" fallbacks; never persisted — news is ephemeral). When an LLM is configured (`userCol('settings').doc('llm')` with `apiKey`, the same doc the Help "Ask AI" checks), an **🤖 AI read** button appears. It sends ONLY on-screen evidence (setup description + quality metrics + the headlines) through the shared `chatCallOpenAICompat` helper and renders the reply in a bordered box with a fixed disclaimer: *"AI draft — not financial advice. The tool assembles evidence; the decision is yours."* **The LLM NEVER gives buy/sell/hold advice or price predictions** — two verbatim system-prompt constants enforce this: dip candidates get an *EMOTIONAL vs STRUCTURAL* read (is the decline sentiment-driven or fundamentally impaired?), non-dip setups get a *premise SUPPORTED vs CONTRADICTED* read. Output is a fixed shape: a `Read:` line, 2–4 evidence bullets, a `Watch for:` line. The button is hidden entirely when no LLM is configured; errors render inline (never `alert`).
