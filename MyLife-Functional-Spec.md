...
- **⚖ Weight Chart accordion**: appears above Last 7 Days in month view. Contains a range dropdown (Last 7 Days / Last 30 Days / This Month / Last 90 Days / This Year / All Time) and a Chart.js line chart of weight over time. Y-axis auto-scales to floor(min−20) → ceil(max+10). Line connects across days with no weight entry. Hover tooltip shows full date + weight. Sticky open/closed and range in `settings/exercisePrefs.dmWeightChartOpen` / `.dmWeightChartRange`. Separate Firestore query independent of the monthly metrics table.
