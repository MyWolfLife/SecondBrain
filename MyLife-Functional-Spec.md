...
- Subcollections per project: `todoItems` ({text, done, notes, sortOrder}), `planningGroups` ({name, sortOrder, items[]}), `days`, `bookings`, `bookingPhotos`, `itemPhotos`, `itemPhotoData`, `packingItems`, `projectNotes`, `receipts` ({description, imageData, createdAt})
...
- **Receipts**: A running log for tracking trip receipts. Add via 🖼️ Add Image (file picker, supports multi-select), 📷 Camera (capture), or 📋 Paste (clipboard) — right after the image is added, a prompt asks for a short description (e.g. "Dinner Wed night"). Receipts stored in `receipts` subcollection (Base64 image + description + createdAt). Displayed as a list of description rows (not a thumbnail grid) — clicking a description opens a **lightbox** with the full image, description, ✏️ Edit Description button, 🗑️ Delete button, and ✕ Close. Editing uses a `prompt()` for simplicity, same as Photos' caption editing. Visible in both planning and travel modes. Accordion summary shows receipt count. Not currently included in project Export or the Print/Save-as-HTML report — only in the full account backup.
...
| `lifeProjects/{id}/receipts` | description, imageData, createdAt |
