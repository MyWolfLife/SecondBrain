...
**Fixed-months schedules** (`recurring.type === 'fixed_months'`, e.g. "fertilize in May, July, and October"): one independent occurrence per configured month, every year. Configured with `months` (number[], 1-12), `dayOfMonth` (number), and `minSpacingDays` (number — captured but not yet enforced; will drive an automatic "Unnecessary" status on a too-soon next occurrence in a later phase). Per-occurrence delete still uses the ordinary `cancelledDates[]` array, but completion status does not (see below).

**Occurrence status** (`occurrenceStatus` map, `reset_interval`/`fixed_months` only — replaces `completedDates` for these two types): keyed by occurrence date string, value is `{ status: 'completed' }` or `{ status: 'in_progress', startedAt, notes }`.
