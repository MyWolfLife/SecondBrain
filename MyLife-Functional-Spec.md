...
- **Evidence chips**: base rate, RSI 14, 5d/60d volume ratio, 60d realized vol, above/below 50d + 200d averages, 52-week range, the earnings-catalyst chip **⚠️ Earnings {date} (±{x}% history)** when a report is stamped (Stage 2.4), plus the Stage 2.2 quality/insider/falling-knife chips (same `_asQualityChips` set as the scan card).
...
- **Metric info popups (Stage 3.5 UX pass)**: every row in the Quality and Analyst view grids (plus the insider-purchases and grade-actions summary lines) carries a small **?** button (`_adInfoBtn(key)`) that opens the shared `#adInfoModal` via `_adShowInfo(key)`. Content comes from the `AD_INFO` glossary in `analyzer-scan.js` — each of the 12 entries has a `simple` explanation (what it is, why it matters, good vs bad values, always shown) and an optional `deep` explanation (source field/formula/caveats, shown below a divider). Reuses the standard `openModal`/`closeModal` utilities.
