...
- **Last 7 Days accordion**: appears at the top of the month view only when the selected month + year match the current month + year. Fetches up to 7 records from a separate Firestore query (date range: today−7 through yesterday — never includes today) — completely independent of the monthly data, so cross-month days never affect the monthly count or summary. Header shows "Last 7 Days" + record count. Content is a single summary card showing the same fields as the monthly summary card, with boolean custom metrics shown as `X / 7` (always out of 7 regardless of how many records exist). Accordion open/closed state is sticky — saved to and loaded from `settings/exercisePrefs.dmLast7Expanded`.
