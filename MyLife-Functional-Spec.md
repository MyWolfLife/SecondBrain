...
- **FMP provider (`js/analyzer-fmp.js`, Stage 3.1)**: `anaFmpGetKey()` (module-cached read of `settings/investments.fmpApiKey`; `anaFmpResetKey()` to force re-read), `_anaFmpGet(pathAndQuery)` (single choke-point: appends `apikey`, 12s timeout, `resp.text()`-first so plain-text 402 bodies don't throw, one 3s retry on 429, and `402 → Error('FMP plan does not include: …')`), and `anaFmpHistory(ticker, from, to)` → the same `{dates,open,high,low,close,volume}` ascending fragment as the Yahoo parser (FMP returns **descending** — reversed here; dots→dashes in the URL only, `BRK.B`→`BRK-B`).
...
- **Hub UI**: "📊 Price data" section on `#analyzer` — cache stats note (tickers, candle count, fresh-today, last update; appends "⚡ FMP fast path active" when an FMP key is set), **Update price data** button running the effective universe + market tickers as a kicked-off job with progress bar (`n / total — TICKER (status)`), Cancel button, and a completion summary incl. per-ticker failures (first 20). The tab must stay open during a run; a full first fetch takes several minutes on the free path, ~2–4 min with an FMP key.
...
- **Catalyst map (Phase 2, Stage 2.4)**: one **Finnhub** earnings-calendar call per scan (`today … today+windowDays`, all US symbols in one request) is the PRIMARY source; the old FMP call is a silent fallback only if Finnhub throws (no key / rate limit). Every shortlisted candidate with an upcoming report in the window gets `earningsDate` stamped plus `eventMovePct` = `anaEngTypicalEventMovePct(rec)` (the stock's own typical earnings-day reaction, from candles). Chip: amber **⚠️ Earnings {date} (±{x}% history)** — the ± figure sizes the binary-event risk, which is the user's call. Candidates with no report in the window show no chip (correct). With Finnhub, coverage is effectively the whole market (~1,500 symbols report in a 60-day window) vs FMP free's ~72.
