...
- **Receipts**: A running log for tracking trip receipts and spending. Add via 🖼️ Add Image (file picker, supports multi-select), 📷 Camera (capture), 📋 Paste (clipboard), or 📝 No Photo (`_lpReceiptAddNoPhoto` — skips straight to the Details modal for a receipt with no image, e.g. a cash tip with nothing to photograph) — right after the image is added (or immediately, for No Photo), a **Receipt Details** modal (`_lpReceiptDetailsPrompt`) asks for **Description** (text, e.g. "Dinner Wed night"), **Date** (date picker, defaults to today, editable), **Amount** (number), and **Category** (dropdown seeded with `LP_RECEIPT_CATEGORIES`: Meals, Snacks, Parking, Souvenirs, Alcohol, Gas, Activity, Other). Receipts stored in `receipts` subcollection (Base64 image, or empty string for No Photo entries + description + date + amount + category + createdAt). List is sorted by date descending (`_lpReceiptSortKey`; older receipts saved before the date field existed fall back to `createdAt` for ordering). Displayed as a list of rows — description, then date, category, and amount — with ✏️ Edit and 🗑️ Delete buttons per row; the description is only clickable (opens the lightbox) when the receipt has an image, otherwise it's plain text. Editing re-prompts the same Details modal pre-filled with the receipt's current date (not reset to today), and re-sorts the list if the date changed. The **lightbox** (image receipts only) shows the full image, description, date + category + amount, ✏️ Edit button (reopens the same Details modal, pre-filled), 🗑️ Delete, and ✕ Close. **Accordion summary** (`_lpReceiptsBreakdownText`) shows the running total followed by a per-category breakdown in parentheses — e.g. `$142.50 (Meals: $80.00, Parking: $20.00, Souvenirs: $30.00, Snacks: $12.50)` — same style as the Trip Info cost rollup breakdown; empty/zero-total shows no summary. Visible in both planning and travel modes. Included in project **Export** (only when the include-photos option is checked, same as `projectPhotos`) and in the **Print/Save-as-HTML** report (its own section, with the same total + breakdown line, placed after Bookings).
