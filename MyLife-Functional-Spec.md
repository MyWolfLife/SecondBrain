...
- **⚖ Weight Chart accordion**: appears above Last 7 Days in month view. Contains a range dropdown (Last 7 Days / Last 2 Weeks / Last 30 Days / Selected Month / Last 90 Days / This Year / All Time), a **Reduce Width %** number input, and a Chart.js line chart of weight over time. The chart width auto-scales to ~35px per data point, treated as a minimum of 7 points so early-month/sparse views (e.g. 1-3 days) don't render as a razor-thin sliver (capped at container width); the Reduce Width input lets the user tighten a sparse chart — entering a percent shrinks the current width by that amount **cumulatively** (e.g. 10 → 90%, enter 10 again → 81%), and the field resets to 0 after each entry. Changing the date range (a full re-render) restores natural width. The shrink factor is not persisted. Y-axis auto-scales with ±1 lb padding for all ranges. Gridline step is 1 lb for short ranges (≤31 days) and 5 lb for longer ranges. By default the x-axis contains one slot per weigh-in, so days with no weight entry are skipped and the line connects consecutive weigh-ins directly. Hover tooltip shows full date + weight + 3-Day Avg. A **Show Projected Weight** checkbox (next to Reduce Width %, tooltip "This is what the weight would be each day according to calorie +/-") toggles a calorie-driven projection line. A **Show Missing Days** checkbox (default off, sticky) expands the x-axis to one slot per calendar day between the first and last plotted point: days without a weigh-in appear on the axis but get **no dot** (null value); all lines (Weight / 3-Day Avg / Projected / Goal) bridge those gaps via `spanGaps`, so the Goal line still passes straight through empty days and the 3-Day Avg keeps its existing entry-based math (unchanged values, just stretched onto true date spacing). When off, behavior is exactly as before (missing days skipped). Up to four datasets: blue line (actual weight, dots clickable → navigates to that day's entry form), orange line (3-day rolling average — pt0=itself, pt1=avg of first 2, pt2+= avg of 3), **light-blue dotted line (Projected)** when the checkbox is on, and **dashed green line (Goal)** when range is "Selected Month" and a goal weight is configured. Goal line: anchored at the first weigh-in of the month, slopes down linearly to the month's goal weight on the last day of the month. When the goal line is shown, Y min/max widen to 1 lb beyond the lowest/highest value across BOTH the actual weight line and the goal line (not just the first/last point), so no point on either line is clipped against the chart edge. **Projected line**: anchored at the first weigh-in **on or after the earliest in-range date that has `foodCalories` logged** (weigh-in points before that date are `null` so the line doesn't draw a useless flat run before any calorie data exists), then each weigh-in's projected value = anchorWeight − (cumulative `totalBurn − foodCalories` for all in-range days from the anchor up to the day before that weigh-in) ÷ 3500. Days missing burn or food count as 0 change (line carries flat across gaps); the Y range widens to include projected values (ignoring the leading nulls) so the line isn't clipped. A Chart.js **legend (key)** is displayed at the top (4 lines now warrant it); the tooltip labels each point with its dataset name. Sticky open/closed, range, projected-on, and show-missing-days in `settings/exercisePrefs.dmWeightChartOpen` / `.dmWeightChartRange` / `.dmWeightChartProjected` / `.dmWeightChartShowMissing` (projected and show-missing default false). Separate Firestore query independent of the monthly metrics table.
...
- **Weight Chart**: collapsible accordion reusing the Daily Metrics chart component (`_dmRenderWeightChart` with unique element ids to avoid duplicate-id collisions). Range selector defaults to **Selected Year** (year-to-date for the current year, full calendar year for a past year); also offers Last 30/90 Days, This Year, All Time. Sticky prefs `sumWeightChartOpen` / `sumWeightChartRange`; shares the `dmWeightChartProjected` projected-weight and `dmWeightChartShowMissing` show-missing-days toggles.
