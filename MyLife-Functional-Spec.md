...
- **Desktop (≥700px)**: scrollable `<table>` with a tinted **summary row** above the column headers. Weight column shows the **net change** (newest − oldest weight in range) in green if negative (lost weight) or red if gained; all other standard fields show averages. "X / N" counts for boolean custom metrics. A computed **+/- Diff** column appears between Burn and Food Cal. showing `burn − food` per row — yellow background/black text when negative (ate more than burned), "—" when positive. Each data row is clickable — navigates to `#exercise-metric/<date>`. Note icons (📝) appear inline with a hover tooltip.
