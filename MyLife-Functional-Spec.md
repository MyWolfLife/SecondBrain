...
**Print / Save as PDF** (`_lpPrintProject` → `_lpBuildPrintDocument`): a "🖨️ Print" button in the project detail page header, next to Export. Opens a **new browser tab** with its own fully standalone HTML document (own `<style>`, no dependency on the app's CSS or nav) and calls `window.print()`, where "Save as PDF" is a built-in destination in every browser's print dialog — no PDF library involved. The target window is opened **synchronously, before any `await`** (`window.open('', '_blank')` as the very first line of `_lpPrintProject`), so browsers don't treat it as a blocked popup; it's filled in with a "Preparing…" placeholder immediately, then the real document once `_lpBuildPrintDocument`'s async reads finish. If the popup is blocked anyway (browser setting), an alert asks the user to allow pop-ups and retry. Like Export, everything is read **fresh from Firestore** rather than from the page's cached globals, and unlike Export, photos are always included (no toggle) since the request that drove this feature was "everything including all pictures." **Every blank/unfilled field is omitted** — a missing cost, confirmation, contact, note, fact list, or photo set simply doesn't render that line, and a whole section (Bookings, Packing, To-Do, Notes, Photos) is skipped entirely if the project has no data for it.
- **Sections, top to bottom**: a summary (title, date range, description, travelers, total cost with a bookings/itinerary breakdown when non-zero, project reference links) → **Itinerary**, day by day, every item fully expanded (status/type/sub-type badges, time/duration/leave-arrival-check-in time, cost, confirmation, contact, booking name, the sub-type-specific downloaded checkbox, notes, **facts**, and **item photos**) with an inline travel-summary line between consecutive on-timeline items (depart time, computed arrival, drive/fly/walk/bike mode + time + miles, route) → **Bookings** (all fields + screenshots) → **Packing List** (grouped by category, checkbox marks) → **To-Do** (checkbox marks) → **Notes** (with facts) → **Photos** (the project-level gallery) → **Locations**, a full data dump of every field for every location linked to the project (this repeats address/phone/lat-lng even where they already appeared inline under an item, by design — a single reference list at the back) → **Distances** (a From/To/Time/Miles/Mode table). The **Planning Board and People are intentionally left out** — Planning Board is undecided/exploratory content that doesn't belong in a "bring on the trip" document, and people are covered by the one-line Travelers list in the summary instead of their own section.
