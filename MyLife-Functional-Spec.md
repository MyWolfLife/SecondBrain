...
**Linked Payments roll-up**: The expanded detail of a Payment Methods item shows a collapsible **Linked Payments** accordion at the bottom, listing every credential (across all people) whose Payment Method points to it. Each row: name · frequency · amount, with the owner's name appended when it differs from the payment method's owner (e.g. `Apple TV (Danielle)`). **One Time** charges are excluded entirely; **Other** charges are listed but not summed. The header shows the count of listed items and recurring totals: `$X/mo · $Y/yr` (Monthly counts as amount/mo and amount×12/yr; Yearly as amount/yr and amount÷12/mo). Totals are hidden when there are no recurring charges.
