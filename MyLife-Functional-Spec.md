...
- **Firestore**: `analyzerScans` — one doc per scan: `{createdAt, date, params, regime{}, funnel{}, durationMs, candidates[{ticker, detector, trigger stats, baseRate, cond*, earningsDate, dismissed, quality{}, insiders{}}]}` (`quality`/`insiders` on dip candidates only, Stage 2.2). In Settings backup.
