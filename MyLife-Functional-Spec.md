...
- **Receipts**: A running log for tracking trip receipts and spending. Add via 🖼️ Add Image (file picker, supports multi-select), 📷 Camera (capture), or 📋 Paste (clipboard) — right after the image is added, a **Receipt Details** modal (`_lpReceiptDetailsPrompt`) asks for **Description** (text, e.g. "Dinner Wed night"), **Amount** (number), and **Category** (dropdown seeded with `LP_RECEIPT_CATEGORIES`: Meals, Snacks, Parking, Souvenirs, Other). Receipts stored in `receipts` subcollection (Base64 image + description + amount + category + createdAt). Displayed as a list of rows — description (clickable, opens the lightbox), then category and amount — with ✏️ Edit and 🗑️ Delete buttons per row. The **lightbox** shows the full image, description, category + amount, ✏️ Edit button (reopens the same Details modal, pre-filled), 🗑️ Delete, and ✕ Close. **Accordion summary** (`_lpReceiptsBreakdownText`) shows the running total followed by a per-category breakdown in parentheses — e.g. `$142.50 (Meals: $80.00, Parking: $20.00, Souvenirs: $30.00, Snacks: $12.50)` — same style as the Trip Info cost rollup breakdown; empty/zero-total shows no summary. Visible in both planning and travel modes. Included in project **Export** (only when the include-photos option is checked, same as `projectPhotos`) and in the **Print/Save-as-HTML** report (its own section, with the same total + breakdown line, placed after Bookings).
