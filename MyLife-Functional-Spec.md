...
4. **Name search**: User can type in the search box; results come from Foursquare biased to the user's current GPS position. Each result shows name, address, category, and **distance from current location** (e.g. "0.3 mi") to disambiguate same-named locations. The **location box** (default "Current location") auto-selects its contents on focus (`setTimeout` deferred past mouseup) so the user can type a city/area straight over it.
