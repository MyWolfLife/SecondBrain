...
- **Evidence chips**: base rate, RSI 14, 5d/60d volume ratio, 60d realized vol, above/below 50d + 200d averages, 52-week range, the earnings-catalyst chip **⚠️ Earnings {date} (±{x}% history)** when a report is stamped (Stage 2.4), plus the Stage 2.2 quality/insider/falling-knife chips (same `_asQualityChips` set as the scan card). All chips and the header badge are tappable per the scan page's `AS_CHIP_INFO` registry (hover tooltip + tap-for-popup); a small "💡 Tap any tag above for what it means." hint (`.as-chip-hint`) sits under the chip row for discoverability.
