...
**Status: ✅ PHASE 1 COMPLETE (all 9 stages, 2026-07-10). ✅ PHASE 2 COMPLETE (all 5 stages, 2026-07-11 — Finnhub enrichment). ✅ PHASE 3 COMPLETE (all 6 stages, 2026-07-12 — FMP Starter: tier map, preferred price provider, divergence, Detector C, Discover screener, consolidation & guardrails). Remaining plan work: strategy-profile UI + holdings check (Goal 2).**
...
- **2026-07-13 — Post-Phase-3 enhancement: FMP fallback tier in the Investments price-update pipeline (not Analyzer stage work).** User reported mutual fund price failures (FXAIX, VTTHX, RDFTX) on "Update prices" — worked fine on their personal PC but not their work PC, pointing to a network-level cause rather than data coverage. `investments.js` gained a middle tier between the existing Finnhub → Yahoo chain: `_investFmpFallback(tickers, priceMap)` / `_investFetchPriceFmp(ticker)`, reusing the Analyzer's own `anaFmpGetKey`/`anaFmpHistory` (same per-user `settings/investments.fmpApiKey`, no new key needed). Real value isn't really data coverage (verified inconsistent — see below) but that FMP is called **directly, no CORS proxy**, unlike the Yahoo tier which routes through public proxy domains that a work-network firewall/security tool (Zscaler etc.) commonly blocks — matching the user's exact symptom. No-op (all tickers pass through unchanged) when no FMP key is configured, so the free path is unaffected. Wired into all three price-update call sites (`_investUpdateAccountPrices`, `_investUpdateAllPrices`, `_investUpdateStocksAllPrices`). Bumps: investments.js v589, sw v477.
  - **Verified (preview, test account, live FMP key):** direct test against the user's exact three tickers — **FXAIX ✅ $263.26, VTTHX ✅ $29.70, RDFTX ❌ no data on FMP at all** — confirms this is a genuine fallback (fixes 2 of 3), not a complete replacement for the Yahoo tier or the Cloudflare Worker (still the only guaranteed fix, already documented in-app). No-key path hand-verified (monkey-patched `anaFmpGetKey` to return `''` — ticker passed through unresolved, unchanged behavior). Full regression pass: ran the real `_investUpdateStocksAllPrices()` button against existing GRAB/NVDA holdings (both resolve via Finnhub, never touch the new tier) — "✓ All 2 prices updated successfully," no console errors, no behavior change to the primary path.
- **2026-07-13 — Post-Phase-3 enhancement: Stock Rollup ↔ dossier bridge.** Not a numbered stage — ad-hoc improvement requested after Phase 3 closed. `investments.js`: **🎯 Show dossier** button in each expanded Stock Rollup ticker row (`.ist-dossier-row`, above the account sub-rows) → `_investOpenDossier(ticker)` → `#analyzer/dossier/none/{ticker}/dipA`. `analyzer-scan.js` (`loadAnalyzerDossierPage`): sentinel `scanId='none'` triggers the dossier's existing read-only deep-link mode (no real scan doc, so no thesis/exit save, no trade ticket); origin-aware breadcrumb + "← Back to…" link/read-only note (Scan vs Stock Rollup, based on `scanId !== 'none'`); **auto-fetch on demand** — if the ticker isn't in the Analyzer's own price cache yet (common for a holding never scanned/updated there — Stock Rollup uses a separate live-quote path), calls `_anaUpdateTicker(ticker)` (single-ticker FMP/Yahoo fetch) before giving up, with an honest failure message naming likely causes instead of the old blanket "run Update price data first" (which assumed a Scan-page visitor). Bumps: investments.js v588, analyzer-scan.js v16, styles.css v772, sw v476.
  - **Verified (preview, test account):** button renders in the expanded row; click navigates to `#analyzer/dossier/none/GRAB/dipA`; dossier renders with correct origin-aware breadcrumb ("Stock Rollup › GRAB"), honest "setup no longer active" badge, read-only note ("opened from Stock Rollup rather than a scan…"), and "← Back to Stock Rollup" link. **Auto-fetch proven end-to-end**: deleted GRAB from the IndexedDB price cache, re-opened the dossier — showed "Fetching price history for GRAB…", then rendered fully (1,254 candles freshly cached, `updatedAt` = now). **Failure path proven**: invalid ticker `ZZZZFAKE99` → "Fetching…" → honest "Couldn't fetch enough price history… invalid ticker, too newly listed, or not covered" message, no dead end. No console errors in any path.
- **2026-07-12 — 🏆 Stage 3.5 COMPLETE → PHASE 3 COMPLETE (consolidation, guardrails, close-out).** `analyzer-fmp.js`: session quota guardrails in `_anaFmpGet` — counts every call (`anaFmpCallCount()`), tallies 402s, and after 3 plan-block 402s flags the session plan-limited (`anaFmpPlanLimited()` → true); `anaFmpResetCounters()`; 402 message reworded to **"Not included in the current FMP plan: {feature}"** (names the feature, at the source so every surface inherits it). New drop-in FMP fallbacks matching the Finnhub shapes: `anaFmpEarningsCalendar(from, to)` and `anaFmpInsiders(ticker, fromDate)` (both full-coverage on Starter). `analyzer-data.js`: unified providers `anaEarningsCalendar(from, to)` + `anaInsiders(ticker, fromDate)` — **Finnhub PRIMARY, FMP fallback** (documented single ordering); every scan/dossier call site now uses these wrappers. `analyzer-scan.js`: swapped all `anaFinnhub{EarningsCalendar,Insiders}` call sites to the wrappers; deleted the ad-hoc raw-fetch `_asFetchEarningsMap` (its logic now lives in the choke-pointed `anaFmpEarningsCalendar`); `anaFmpResetCounters()` at scan start; the FMP analyst-enrichment loop checks `anaFmpPlanLimited()` and **truncates cleanly** (labels remaining cards "analyst data not included in the current FMP plan") instead of failing candidate-by-candidate. `analyzer.js`: hub **provider-health line** (`✓/— FMP · Finnhub · Yahoo worker · public proxies`) from config presence only, no API calls. Bumps: analyzer-fmp.js v4, analyzer-data.js v7, analyzer.js v11, analyzer-scan.js v11, sw v470. No new collection.
  - **Verified (preview, test account, live FMP + Finnhub):** all Stage 3.5 symbols defined and page loads clean (no console errors). Provider-health line renders **"✓ FMP · ✓ Finnhub · — Yahoo worker · ✓ public proxies"** (both keys set, no worker, proxies always ✓). `anaFmpEarningsCalendar` (30-day window) → **3,892 symbols**, correct shape (date/epsEstimate/epsActual/revenueEstimate/revenueActual) — Starter full-coverage confirmed. `anaFmpInsiders('AAPL', −1y)` → 26 buys / 74 sells / 773,332 buyShares, 0 open-market P- purchases (realistic for AAPL; Finnhub-identical shape). FMP call counter increments (2 after two fetches); `anaFmpResetCounters()`/`anaFmpPlanLimited()` work. **Unified ordering proven:** `anaEarningsCalendar` returned **1,498 symbols with 0 FMP calls** — Finnhub answered as primary, FMP untouched. Plan-limited back-off logic hand-verified (3×402 ⇒ flag ⇒ enrichment loop `continue`s with a labeled note). No overflow.
  - **Exit criteria (all met):** fast parallel price refresh on Starter ✅ (Stage 3.1); divergence chip live once snapshots cover a dip ✅ (3.2); weekly snapshots auto-write ✅ (3.2); Detector C armed, fires on synthetic data ✅ (3.3); Discover mode with 2,000 cap ✅ (3.4); unified earnings/insider providers with one documented order + clean plan-limited degradation ✅ (3.5); free/no-key path unchanged (Finnhub-primary wrappers fall back only when a key exists; provider line shows proxies-only) ✅. **Phase 3 closed.** Remaining plan work (separate effort): strategy-profile UI (threshold config) + holdings check (Goal 2).
- **2026-07-12 — ✅ Stage 3.4 COMPLETE (Discover mode — market-wide universe expansion).** `analyzer-fmp.js`: `anaFmpScreener(minCap, minVol)` (company-screener, ETFs/funds excluded, sorted by market cap, **hard-capped 2,000**). `analyzer.js`: config gains `discoverEnabled/discoverMinMarketCap/discoverMinVolume/discoverList[{t,n}]/discoverFetchedAt`; `_anaRefreshDiscoverList(force)` caches the screener list weekly (or on force); `_anaEffectiveUniverse` unions the discover list when enabled; Universe-page **🔭 Discover mode** card (toggle + $cap/vol inputs + 2,000-cap note + list count/date + Refresh button) with handlers `_anaToggleDiscover`/`_anaSaveDiscoverThresholds`/`_anaRefreshDiscoverNow`. `analyzer-scan.js`: `_asName` also resolves non-S&P names from the discover list. Downstream (price update, scan, backtest) iterates the effective universe unchanged. Bumps: analyzer-fmp.js v3, analyzer.js v10, analyzer-scan.js v10, sw v469. Config collection already in the Settings backup — no settings.js change.
  - **Verified (preview, test account, live FMP):** screener returned **1,046 companies** at defaults ($2B cap / 1M vol, ETFs excluded); enabling Discover grew the effective universe **504 → 1,176**; `_asName('TSM')` → "Taiwan Semiconductor…" (non-S&P name resolved from the list); a scan ran cleanly over the 1,176-ticker universe (uncached names skipped, no errors, 4.7s); `navigator.storage.estimate()` = 0.9MB used of 3,138MB quota (0.03%) with 9 tickers cached → ~500MB projected at the full 2,000, well within quota; disabling Discover returned the universe to **504**. Discover card renders (toggle, $2B/1M inputs, 2,000-cap note). No console errors, no overflow. Sandbox: Discover left **disabled** (the ~1,046-name `discoverList` stays cached on the config doc — designed weekly-cache behavior, harmless).
  - **Next: Stage 3.5** — consolidation & guardrails (FMP-vs-Finnhub source ordering for earnings/insiders per the 3.0 tier map, per-session FMP quota guards, 402 "not in your plan" messaging, provider-active line on the hub) → Phase 3 close-out.
- **2026-07-12 — ✅ Stage 3.3 COMPLETE (Detector C: estimate-revision momentum).** Wired the already-built pure `anaEngRevisionTrigger` into the scan. `analyzer-scan.js`: loads the last 12 weekly snapshot docs once per scan; `_asExtractEstSeries(docs, ticker)` builds each ticker's ascending `{date,eps,analysts}` series; main loop runs the trigger (needs ≥3 snapshots ≥28d apart, est ≥+3%, price lagging, ≥3 analysts) → `revC` candidates ranked by `gapPts`, cap 15, own **📈 Revision momentum** section; card badge `est {e}% vs price {p}%` + reason + gap chip; dossier revC variant (estimate-history table, no dips table); AS_DEFAULTS gains `revMinEstPct/revMinSpanDays/revMinAnalysts/revMaxWeeks`. Label maps updated everywhere: `AS_DET_LABELS`, `AB_DET_LABELS`, backtest inline map, scoreboard `ASB_DET_SHORT` (all gain revC/'Revision'); scan render loop includes `revC`. Backtest: **disabled** Detector C checkbox + "needs accumulated estimate history" note (no consensus history before our snapshots). Bumps: analyzer-scan.js v9, analyzer-backtest.js v5, analyzer-scoreboard.js v3, sw v468. (Engine `anaEngRevisionTrigger` already built + verified as groundwork — no engine change.)
  - **Verified (preview, test account, synthetic snapshots):** seeded 4 weekly FLEX snapshots (eps 4.00→4.40 = +10%, 28-day span); scan stamped `revC {est +10, price −9.96, gap +19.96, weeks 4}` = exact hand-check; badge "est +10.0% vs price −10.0%", reason + "Est gap: +20.0 pts" chip; FLEX correctly appears under BOTH dipA and revC. Dossier revC variant renders the 4-row estimate-history table, no dips table. Scoreboard shows "FLEX / Revision". Backtest form shows the disabled Detector C checkbox + note. **Zero-candidates-on-real-data confirmed:** after deleting the synthetic snapshots, a scan produces 0 revC (the "arms itself" behavior — correct). No console errors, no overflow. Synthetic snapshots + test scan deleted; sandbox restored (2 real scans, 0 estimate snapshots).
  - **Next: Stage 3.4** — Discover mode (screener expands the effective universe to ≤2,000 liquid names; local detectors run across it unchanged). `anaFmpScreener` (verified ✅ in 3.0 → 1,267 names at defaults), Universe-page Discover card, `_anaEffectiveUniverse` union, storage/quota checks.
- **2026-07-12 — ✅ Stage 3.2 COMPLETE (estimates layer + weekly snapshots + divergence chip — the flagship).** `analyzer-fmp.js`: `anaFmpEstimates` (annual; picks the FY whose end ≥ today as epsCurrY, next as epsNextY, stamps `fyLabel`=FY-end for the rollover guard), `anaFmpPriceTarget`, `anaFmpGrades(ticker, sinceDate)` (filters/slices client-side since `limit` is ignored). New collection **`analyzerEstimates`** — one doc/week (id = week's Monday date): `{weekId, date, createdAt, count, data:{TICKER:{epsCurrY,epsNextY,numAnalysts,fyLabel}}}`; written by `anaFmpSnapshotEstimates` (pool of 5, throttled, overwrites same-week, refuses empty doc); helpers `anaEstCurrentWeekHasSnapshot`/`anaEstCountSnapshots`/`anaEstGetLatestSnapshot`/`anaEstGetSnapshotOnOrBefore`. **Added to settings.js backup list + spec Part 15.** `analyzer-scan.js`: shortlist enrichment stamps `estimates`/`priceTarget`/`grades`; dip candidates compute `anaEngDivergence` (snapA on/before peak, snapB latest) → stamp `divergence` or honest `divergenceNote`; `_asAnalystChips` renders the flagship divergence chip FIRST (green when ≥5 pts) + target + grades chips; dossier gains a **🧮 Analyst view** section (`_adRenderAnalyst`); `_asMaybeSnapshotEstimates` auto-records the week's snapshot after a scan (fire-and-forget). `analyzer.js`: hub **📸 Snapshot estimates** button (`_anaRunEstimateSnapshot`). Bumps: analyzer-fmp.js v2, analyzer-scan.js v8, analyzer.js v9, settings.js v662, sw v467. (Engine `anaEngDivergence` was already built + verified as groundwork — no engine change.)
  - **Verified (preview, test account, live FMP + synthetic snapshots):** fetchers — AAPL epsCurrY 8.755 (FY2026)/epsNextY 9.64/31 analysts/fyLabel 2026-09-27, target 327, grades 0▲/1▼/11= last 60d. Snapshot job — wrote week doc, re-run overwrote (1 doc, no dup). **Divergence hand-verified end-to-end:** seeded FLEX snapshots 3.00 (pre-peak 2026-05-01) → 2.94 (2026-07-06); scan stamped `{est −2.0, price −16.21, pts +14.21, analysts 9}` = exact match; chip **"Est −2.0% vs price −16.2% → +14.2 pts emotional"** leads the row (green); Target $150 (+10.5%) chip. Honest path — 0 snapshots → "Divergence: needs 2 more weekly snapshots" (never faked). Dossier 🧮 Analyst view renders (EPS 4.43/6.78, target $150 range $80–$203, grade table). Backward-compat — old fixture scan renders chip-free. Hub 📸 button present. No console errors, no overflow. Synthetic snapshots + test scan deleted; sandbox restored (2 real scans, 0 estimate snapshots).
  - **Next: Stage 3.3** — Detector C (estimate-revision momentum). Engine core `anaEngRevisionTrigger` already built + verified as groundwork; needs scan wiring (new `revC` detector, label maps incl. the scoreboard/backtest), a dossier variant, and it "arms itself" only after ~4 weekly snapshots exist (verify with synthetic snapshot series; no backtest).
- **2026-07-12 — ✅ Stage 3.1 COMPLETE (FMP as preferred price provider).** New `js/analyzer-fmp.js`: `anaFmpGetKey`/`anaFmpResetKey` (module-cached key), `_anaFmpGet` choke-point (12s timeout, text-first 402 parsing, one 3s 429 retry, `402 → "FMP plan does not include"`), `anaFmpHistory(ticker,from,to)` (reverses FMP's DESCENDING response → ascending; dots→dashes URL only). `analyzer-data.js`: `_anaRangeToDates` + `_anaFetchHistory` provider chain (FMP if key → Yahoo worker/proxy fallback on ANY error), `_anaUpdateTicker` now calls it; `_anaUpdatePrices` branches to `_anaUpdatePricesParallel` (pool of 5, global 250ms start-spacing ≈240/min, doubles to 500ms on 429) when a key exists — the no-key sequential path is byte-identical. Hub note appends "⚡ FMP fast path active". **Batch-quote fast-path (spec point 4) DROPPED per Stage 3.0** (batch-quote is 402 on Starter) — the parallel history path covers same-day refreshes. Bumps: analyzer-fmp.js v1 (new, +sw precache), analyzer-data.js v6, analyzer.js v8, sw v465.
  - **Verified (preview, test account, live FMP Starter key):** (a) `anaFmpHistory` — AAPL 60 candles ASCENDING, fields aligned; **AAPL 2026-07-10 close 315.32 == Yahoo 315.32 (diff 0.0000)**; BRK.B dotted→dashed works (28 candles); SPY ETF works. (b) parallel path — 3 uncached tickers (MSFT/GOOGL/AMZN) fetched full 5y (1,255 candles each) in **884ms** → ~2 min extrapolated for the full universe. (c) fallback — FMP key blanked (module cache) → META cached via Yahoo path (long-float close = Yahoo signature), key restored; **^VIX (no FMP coverage) transparently falls through FMP→Yahoo → 83 candles, 15.03**. (d) hub note shows "⚡ FMP fast path active". No console errors, no overflow. Sandbox price cache grew to 9 tickers (added NVDA earlier + MSFT/GOOGL/AMZN/META) — benign.
  - **Next: Stage 3.2** — estimates layer + weekly `analyzerEstimates` snapshots → the divergence chip (engine core `anaEngDivergence` already built + verified as groundwork). Uses `analyst-estimates period=annual` (epsAvg, numAnalystsEps, date=FY-end→fyLabel), `price-target-consensus`, `grades`. New Firestore collection → add to settings.js backup + spec Part 15.
- **2026-07-12 — ✅ Stage 3.0 COMPLETE (post-subscription tier map).** User subscribed to FMP Starter on the SAME account/key (32-char `DJuVjnpk…`, already in the test account — no new key). Revalidated every endpoint live (~13 calls). Confirmed on Starter: `historical-price-eod/full` ✅ (DESCENDING order — sort ascending; +vwap/changePercent fields), `company-screener` ✅ **1,267 names** at defaults, `analyst-estimates period=annual` ✅ (epsAvg/numAnalystsEps; date=FY-end), `price-target-consensus` ✅, `grades` ✅ (limit NOT honored → filter by date), `earnings-calendar` ✅ **3,597 symbols/60d** (vs ~72 free), `insider-trading/search` ✅ **now unlocked**, `ratios`/`income-statement` ✅. **STILL 402 on Starter: `batch-quote` and `analyst-estimates period=quarter`.** Legacy `/api/v3/quote/A,B,C` fully retired (403). **Decision gate:** screener ✅ / batch-quote ❌ — reported; user chose to proceed (the two load-bearing endpoints work; only a micro-optimization is lost). Updated the endpoint table + added "C-bis" implications (incl. the REQUIRED Stage 3.1 amendment: no batch-quote → daily fast-path uses parallel single `quote?symbol=` calls). No spec/AppHelp change (nothing user-visible). Commit `docs(analyzer): Stage 3.0 — FMP Starter tier map`, notify, push.
  - **Next: Stage 3.1** — FMP as preferred price provider (parallel `historical-price-eod/full` backfill → ~2–4 min full universe; daily top-up via parallel single quotes; provider chain FMP→Worker→proxies with the free path untouched). NOTE the batch-quote amendment above. Also still pending from earlier: the `analyzer-fmp.js` scaffold groundwork (key reader + `_anaFmpGet` choke-point + `anaFmpHistory`) folds naturally into 3.1 now.
- **2026-07-12 — Phase 3 GROUNDWORK 1/2: engine cores (pre-subscription, not wired in).** Building the tier-independent pieces of Phase 3 BEFORE the user subscribes to FMP Starter (safe because these are pure functions fed PARSED snapshots — immune to the FMP response shape). Added to `js/analyzer-engine.js`: `anaEngDivergence(rec, snapA, snapB, ticker, peakDate, asOfIndex)` (Stage 3.2 flagship math — price-vs-estimate gap; snapshot entry contract `{epsCurrY, epsNextY, numAnalysts, targetConsensus, fyLabel}`; fiscal-rollover guard via `fyLabel`) and `anaEngRevisionTrigger(rec, snapshots, ticker, opts)` (Stage 3.3 Detector C core — rising forward EPS while price lags; needs ≥3 snapshots spanning ≥28d, ≥3 analysts, est ≥+3%). **NOT wired into the scan UI** — that's Stages 3.2/3.3 once a Starter key + snapshot pipeline exist. Bumps: analyzer-engine.js v6, sw v464. No spec/AppHelp change (nothing user-visible yet).
  - **Verified (preview, synthetic data, hand-checked):** divergence — price −15% / est −2% → +13.0 divergence pts, analysts 11; nulls on fiscal-rollover, missing ticker, null EPS. revision — est +10% / price +3% → +7.0 gap pts, 3 weeks; nulls on <3 snapshots, <28d span, <3 analysts, flat estimates, and price-not-lagging. Console clean.
  - **Next groundwork (2/2):** `js/analyzer-fmp.js` scaffold — `anaFmpGetKey`, `_anaFmpGet` choke-point (402-text-first parsing + "not in your FMP plan" error, verifiable against the free key's real 402s) and `anaFmpHistory` (works on free tier for popular symbols). Then everything else waits for FMP Starter + Stage 3.0 tier map.
- **2026-07-11 — 🏆 Stage 2.5 COMPLETE → PHASE 2 COMPLETE (news + AI read).** Dossier (`js/analyzer-scan.js`) gains a **📰 Recent news** section (async `_adRenderNews` → `anaFinnhubNews` last 14d, up to 10 dated/sourced/linked headlines, ephemeral) and, when an LLM is configured (`userCol('settings').doc('llm')` w/ `apiKey`), a **🤖 AI read** button (`_adAiRead`) that sends ONLY on-screen evidence through the shared `chatCallOpenAICompat`, renders in a bordered `.ad-ai-box` with the fixed disclaimer. Two verbatim system-prompt constants: `AD_AI_SYSTEM_DIP` (EMOTIONAL vs STRUCTURAL) for dips, `AD_AI_SYSTEM_GENERIC` (premise SUPPORTED vs CONTRADICTED) for spring/drift; `_adAiDescription(ev)` builds the per-detector one-liner. `_adRenderQuality` now stashes `_adCtx._quality` for the prompt. Never buy/sell/hold — enforced in the prompts. New CSS `.ad-news-list`/`.ad-ai-box`/`.ad-ai-disclaimer`. Bumps: scan v7, styles v769, sw v463.
  - **SecondBrain rule check: NO new SecondBrain LLM action added** (this is a dossier feature calling the shared chat helper, not a natural-language SecondBrain action) → `SB_HELP_ACTIONS` correctly untouched.
  - **Verified in preview (test account).** No-LLM path: news renders 10 real FLEX headlines (2026-07-10 Yahoo "Can Flex Ride the AI Supercomputer Boom…", working links), **AI button correctly HIDDEN**. With an LLM key (user supplied an OpenAI key, model **gpt-5.4-mini**, saved to the test account's `settings/llm`): **dip variant** on FLEX returned in ~seconds — `Read: EMOTIONAL` + 4 evidence bullets (cited the Cerebras headlines + 3.1% margin / 0.73 D/E) + `Watch for:` line + disclaimer, in the bordered box; **spring variant** on EA correctly used the GENERIC prompt → `Read: MIXED/UNCLEAR` (SUPPORTED/CONTRADICTED framing, cited real EA headlines) + disclaimer. No console errors, no overflow. **NOTE: the user's real OpenAI key is now stored in the TEST ACCOUNT's `settings/llm` doc** (per-user isolated) — left in place so the AI read stays testable; the user can remove it via Settings anytime.
  - **PHASE 2 EXIT CRITERIA — all met:** (1) three detector sections on a scan; dip candidates carry quality+insider+falling-knife chips; in-window candidates show ⚠️ Earnings {date} (±x% history). (2) dossier shows Quality, Recent news, and (LLM configured) a working AI read w/ disclaimer; driftB variant renders. (3) Backtest Lab offers driftB as a documented Phase-3 unlock (free tier can't supply historical report dates). (4) Scoreboard renders all three detectors (detector-agnostic labels). (5) every stage committed+pushed w/ spec/AppHelp/bumps, Build Log current, no console errors, old scans still render. (6) zero new collections, zero secrets in repo, all Finnhub calls through the one rate-limited choke-point.
  - **Next (future): Phase 3** — FMP paid. Prereq: user subscribes to FMP Starter (runbook Phase B). Then Stages 3.0–3.5 (tier revalidation, FMP price provider, weekly estimate snapshots → divergence chip, Detector C revision momentum, Discover-mode screener, consolidation).
- **2026-07-11 — Stage 2.4 COMPLETE (catalyst map).** Earnings chip is now **Finnhub-primary** (all US symbols): `_asComputeScan` replaced the FMP-first block with a forward `anaFinnhubEarningsCalendar(today … today+windowDays)` call, FMP (`_asFetchEarningsMap`) kept as a silent fallback only when Finnhub throws. Every shortlisted candidate with a report in the window gets `earningsDate` + `eventMovePct` stamped. New pure engine fn `anaEngTypicalEventMovePct(rec)` (mean of the 5 largest abs daily % moves, rounded). New helper `_asEarningsChipText(c)` → **⚠️ Earnings {date} (±{x}% history)**, reused by card + dossier. Bumps: engine v5, scan v6, sw v462.
  - **Verified in preview (test account, live Finnhub):** engine `anaEngTypicalEventMovePct` cross-checked by hand — FLEX ±18 (manual avg 18.09; top5 39.7/13.9/13.6/12.2/11), EA ±11, SPY ±6 (mega-cap sanity ✓). Forward calendar returns 1,493 symbols; NVDA in-window (2026-08-25), FLEX/EA correctly NOT in window → no chip. Seeded NVDA; card renders "⚠️ Earnings 2026-08-25 (±18% history)"; dossier chip identical. **Full integration**: forced NVDA to trigger → the in-scan stamping loop stamped earningsDate + eventMovePct 18 and the chip rendered (monkeypatch restored). **FMP-fallback path**: blanked the Finnhub key → scan ran without crashing, logs showed "Finnhub earnings calendar unavailable, trying FMP", FLEX/EA rendered chip-less (sparse FMP coverage) → **key restored** (verified 40-char round-trip). No console errors, no overflow. Test scans deleted; sandbox back to 2026-07-11 (2) + 2026-07-10 (20). **NVDA added to the price cache** during testing (benign; cache now SPY/^VIX/FLEX/EA/NVDA).
  - **Next: Stage 2.5** — news feed + optional "🤖 AI read" (emotional vs structural) on the dossier via the app's existing LLM config. First step: ask the user whether the test account should get an LLM key (per standing key protocol); if not, verify the button correctly hides with no config.
- **2026-07-11 — Stage 2.3 COMPLETE (Detector B: post-earnings drift).** New engine fn `anaEngDriftTrigger(rec, earnings, opts)` (pure; EPS beat ≥ minSurprisePct + reaction-day gap-up that held above open ≥ minDay1Pct + inside 1…maxAgeDays window; bmo→report-day index, amc/other→next day). Scan: one trailing earnings-calendar call (`today−21d…today`) up front, per-ticker drift check, `driftB` candidates rank by surprise% desc / cap 15 / own **🚀 Post-earnings drift** section; `_asCandidateCard` driftB branch (badge `beat +X% · day1 +Y%`, reason, revenue-beat chip). Dossier: driftB badge + "📊 Report" line + green-diamond reaction marker on the chart + no dips table; reconstructs live from a 120-day calendar when unstamped. `AS_DET_LABELS`/`AB_DET_LABELS` gain driftB. Bumps: engine v4, scan v5, backtest v4, scoreboard v2, styles v768, sw v461.
  - **FREE-TIER HISTORY FINDING (decides the backtest):** verified live — Finnhub free earnings calendar returns data only for a rolling ~1-month-back-to-forward window: `calendar(2026-01-01…03-31)` (fully-past quarter) → **0 symbols**; `calendar(2026-06-01…06-30)` → 236; `calendar(today…+60d)` → 1,493. `anaFinnhubSurprises` `period` = **quarter-end** ('2026-03-31'), NOT the report date (report lands weeks later, not derivable). ⇒ historical report dates across a backtest span are NOT available on the free tier. **Per the plan's decision gate, the driftB backtest is a Phase 3 unlock** — added as a DISABLED checkbox on the Backtest Lab with an amber "Phase 3 unlock" note explaining why. Detector B runs live on the Scan page (only recent reports needed).
  - **BUG FOUND & FIXED during verification:** the Scoreboard and Backtest signals table used a binary `detector==='dipA' ? 'Dip' : 'Spring'` label, which mislabeled driftB candidates as "Spring". Made both detector-agnostic (`{dipA:'Dip',springD:'Spring',driftB:'Drift'}`); Scoreboard now renders `FLEX / Drift` correctly.
  - **Verified in preview (test account, live Finnhub + engine unit tests):** (a) `anaEngDriftTrigger` unit-tested against FLEX's real 2026-05-08 post-earnings pop (reported 05-07 amc, +6.89%): fires with day1RetPct 6.89 (exact), surprise 10%, reactionDate matches, daysSince 3; returns null when minDay1Pct > move, when no surprise, and when past the window; bmo variant fires on the report day itself. (b) Live scan runs clean, driftB section empty (no cached ticker reported in last 21d — valid). (c) Synthetic driftB candidate (built from the real FLEX reaction) → card badge "beat +10.0% · day1 +6.9%", reason + "Revenue beat too" chip; dossier badge + Report line ("📊 Report 2026-05-07: EPS 1.1 vs 1 est (beat +10.0%) · revenue beat · day-one +6.9% · day 3…"), reaction marker = 1 chart point, NO dips table. (d) Backtest form shows disabled driftB + Phase-3 note; a normal A/D run completes with no driftB signals. (e) Scoreboard renders driftB row as "Drift"; scan page shows all three detector sections. No console errors, no overflow.
  - **Sandbox note:** deleted the synthetic driftB test scan (fabricated EPS) after verification; sandbox back to 2026-07-11 (2 real enriched candidates) + 2026-07-10 (20-candidate fixture). **DEV GOTCHA logged:** the service worker serves JS by URL-ignoring-`?v` (`ignoreSearch:true`), so editing a JS file AFTER its `?v` was already fetched once poisons that version in the browser/HTTP cache — bump to a NEW `?v` number after any further edit in the same session (hence backtest.js reached v4). For clean preview verification, unregister the SW + clear caches, then navigate to `index.html?fresh=N#...`.
  - **Next: Stage 2.4** — catalyst map: replace FMP-first earnings enrichment with Finnhub-primary (all symbols) + `anaEngTypicalEventMovePct` for the "±x% history" binary-event sizing chip.
- **2026-07-11 — Stage 2.2 COMPLETE (quality gate + insider signal).** `js/analyzer-scan.js`: `_asComputeScan` now enriches each DIP shortlist candidate sequentially after the cap (`anaFinnhubMetrics` + `anaFinnhubInsiders(ticker, peakDate)`, per-candidate try/catch, progress note "Enriching n / N — TICKER…"); `quality`/`insiders` stamped on the candidate (ride along on the scan doc). New helper `_asQualityChips(c)` → `[{text, cls, lead}]` builds the chips; `_asCandidateCard` renders falling-knife lead flag + quality/insider chips; dossier `_adRender` adds a **🏥 Quality** section filled async by new `_adRenderQuality()` (stamped data when present, else live Finnhub fetch, "Quality data unavailable" fallback, element-replaced guard). New CSS: `.as-chip-good` (green), `.ad-quality-grid`. NO engine changes, NO new collections. Bumps: analyzer-scan.js?v=4, styles.css?v=767, sw bishop-v460. Spec Part 8f (live-scan + dossier + Firestore candidate shape) and AppHelp (screen:analyzer-scan chips/quality, screen:analyzer-dossier Quality section) updated.
  - **Verified in preview (test account, live Finnhub).** Ran `_asComputeScan` on the 4-ticker cache: **FLEX (dip) enriched** → profitable true, netMargin 3.15%, D/E 0.73, div null, 0 insider 'P' purchases; progress note "Enriching 1 / 1 — FLEX…" fired; **EA (spring) correctly NOT enriched**. Card chips: FLEX → `✅ Profitable` + `Debt/eq 0.7`; synthetic falling-knife candidate → `[LEAD]⚠️ Falling knife?` (amber) + `⚠️ Unprofitable` + `Debt/eq 3.4` (amber) + `👤 Insider buys: 1` (green) — exactly per spec. **Backward compat confirmed**: EA card, a no-enrichment fixture-style card, and the old 2026-07-10 fixture dossier all render cleanly with no quality chips/errors. **Dossier Quality section**: stamped-data path (fresh scan) renders the metric grid inline; **live-fetch fallback** (old fixture FLEX dossier, no stamped quality) fetched from Finnhub and rendered the grid. Persistence: enriched scan saved (`quality`/`insiders` present on dip candidate). No console errors, no horizontal overflow.
  - **Sandbox note**: saved one enriched scan **2026-07-11 (id `7prPG3JmCdygRnczV5Mc`, 2 candidates: FLEX dip enriched + EA spring)** — a useful fixture for testing Stages 2.3+ rendering with real stamped quality data. The 2026-07-10 fixture (id `vj2ZUouu64RFXXzEbnSj`, 20 candidates, no enrichment) is retained as the backward-compat test.
  - **Next: Stage 2.3** — Detector B (post-earnings drift): engine `anaEngDriftTrigger`, scan shortlist section, dossier variant, backtest support (VERIFY free-tier earnings-history depth — may be Phase 3 unlock).
- **2026-07-11 — Stage 2.1 COMPLETE (Finnhub data layer).** New "Finnhub fetchers (Phase 2)" section in `js/analyzer-data.js`: `_anaFinnhubGet` choke-point (key via `_investGetFinnhubKey`, 1100ms spacing, 10s timeout, one 5s retry on 429), `_anaPick`, `anaFinnhubMetrics`, `anaFinnhubInsiders`, `anaFinnhubEarningsCalendar` (earliest-date-per-symbol map), `anaFinnhubSurprises`, `anaFinnhubNews` (unix→date, cap 15). Bumps: analyzer-data.js?v=5, sw bishop-v459. Spec Part 8f updated. AppHelp: evaluated, no user-visible change (pure data layer) — none needed.
  - **All 7 verifications passed against live Finnhub (test account key saved to `userCol('settings').doc('investments')`).** (1) `anaFinnhubMetrics('FLEX')` → 5/6 non-null (netMargin 3.15%, D/E 0.73, currentRatio 1.36, ROE 9.53%, profitable true; dividend null = FLEX pays none). **Key spellings validated against the real 130-key payload — `_anaPick` lists were all correct, NO fixes needed** (`netProfitMarginTTM`, `totalDebt/totalEquityQuarterly`, `currentRatioQuarterly`, `roeTTM` all present). Dividend key confirmed on a real payer: `anaFinnhubMetrics('KO').dividendYieldPct` = 2.57%. (2) `anaFinnhubInsiders('FLEX','2026-04-01')` → 15 buys / 115 sells, 0 open-market 'P' purchases (valid). (3) `anaFinnhubEarningsCalendar('2026-07-11','2026-09-09')` → **1,493 symbols** (blows past FMP's ~72; NVDA 2026-08-25 amc, TGT 2026-08-19). (4) `anaFinnhubSurprises('FLEX')` → 4 quarters w/ actual/estimate/surprisePercent. (5) `anaFinnhubNews('FLEX', −14d, today)` → 15 headlines w/ working URLs. (6) spacing measured at 1,158ms between successive calls (≥1,100ms enforced). (7) 429-retry branch verified by code review (plan-sanctioned; not burning 70 calls). Console clean.
  - **Next: Stage 2.2** — quality gate + insider chips on dip candidates + falling-knife flag + dossier Quality section (`js/analyzer-scan.js`). Uses `anaFinnhubMetrics`/`anaFinnhubInsiders` per-candidate after the top-15 cap.
...
**What Phase 3 buys (the "why" for the implementer):** (1) **Speed** — FMP allows direct browser calls at ~300 req/min on the Starter plan, replacing the free CORS-proxy chain that makes a full price refresh take ~18 minutes (parallel history calls → ~2–4 min full universe). *(Note: batch-quote is NOT in Starter — see the 3.0 tier map; the daily refresh uses parallel single-quote calls instead, still fast.)* (2) **Analyst estimates** — unlocks the plan's sharpest metric: *price fell 15% but consensus forward EPS fell only 2% ⇒ the 13-point gap is the emotional component of the dip, quantified* — plus a new detector (estimate-revision momentum, Detector C). (3) **The screener** — expands discovery beyond the fixed ~505-ticker universe to the whole listed market.
...
**C. Endpoint status table — ✅ CONFIRMED ON STARTER 2026-07-12** (revalidated live from the test account's key after subscription; ✅/❌ are actual observed results, not expectations):
| Endpoint | Free | Starter (confirmed 2026-07-12) | Response shape notes |
|---|---|---|---|
| `historical-price-eod/full?symbol=&from=&to=` | ✅ (popular; 250/day) | ✅ all symbols, no daily cap | array[{symbol,date,open,high,low,close,volume,change,changePercent,vwap}] **DESCENDING** date order (confirmed) — SORT ASCENDING before use |
| `batch-quote?symbols=A,B,C` | ❌ 402 | ❌ **STILL 402 — NOT in Starter** | daily "one call for all symbols" fast-path is unavailable; see 3.0 implications below for the workaround |
| `quote?symbol=` (single) | — | ✅ | [{symbol,name,price,changePercentage,…}] — single-symbol; the per-ticker fallback for same-day price top-up |
| `company-screener?…` | ❌ 402 | ✅ **1,267 names** at defaults (cap>2B, vol>1M, NYSE/NASDAQ/AMEX, active) — under the 2,000 cap | [{symbol,companyName,marketCap,sector,industry,beta,price,lastAnnualDividend,volume,exchange,exchangeShortName,country,isEtf,isFund,isActivelyTrading}] |
| `analyst-estimates?symbol=&period=annual&limit=` | ✅ | ✅ | array[{symbol,date,revenueLow/High/Avg,ebitda*,ebit*,netIncome*,sgaExpense*,epsAvg,epsHigh,epsLow,numAnalystsRevenue,numAnalystsEps}] — `date` = fiscal period END, entries = fiscal YEARS. CURRENT consensus only — NO history (this is why Stage 3.2 builds our own snapshots) |
| `analyst-estimates` `period=quarter` | ❌ 402 | ❌ **STILL 402** ("Premium Query Parameter") | annual only — fine for Stage 3.2 |
| `price-target-consensus?symbol=` | ✅ | ✅ | [{symbol,targetHigh,targetLow,targetConsensus,targetMedian}] |
| `grades?symbol=&limit=` | ✅ | ✅ | array[{symbol,date,gradingCompany,previousGrade,newGrade,action}] — DATED. **`limit` is NOT honored** — AAPL returned 1,771 rows (full history); `anaFmpGrades(ticker, sinceDate)` must filter client-side by date |
| `earnings-calendar?from=&to=` | ✅ but ~72 symbols | ✅ **FULL COVERAGE — 3,597 unique symbols / 60-day range** (vs ~72 free) | [{symbol,date,epsActual,epsEstimated,revenueActual,revenueEstimated,lastUpdated}] — dashes for classes (BRK-B): normalize `.replace(/-/g,'.')` |
| `insider-trading/search?symbol=` | ❌ 402 | ✅ **NOW WORKS on Starter** | [{symbol,filingDate,transactionDate,reportingCik,companyCik,transactionType,securitiesOwned,reportingName,typeOfOwner,acquisitionOrDisposition,directOrIndirect,formType,securitiesTransacted,price,securityName,url}] — can back up / consolidate Phase 2's Finnhub insiders (decide in 3.5) |
| `ratios?symbol=`, `income-statement?symbol=` | ✅ (popular) | ✅ all symbols | ratios keys incl. netProfitMargin, currentRatio, quickRatio, …; income keys incl. revenue, grossProfit, … — quality-gate alternates to Finnhub |

**C-bis. Stage 3.0 decision-gate outcome + implications for later stages:**
- **Decision gate result:** the plan said STOP if `batch-quote` OR `company-screener` are still 402 on Starter. **Screener ✅ works; batch-quote ❌ does not.** Reported to the user, who chose to proceed — the value case holds because the two load-bearing endpoints (5-year history for fast backfill, screener for discovery) both work; only the batch micro-optimization is lost.
- **Stage 3.1 amendment (REQUIRED):** there is NO batch-quote on Starter, and FMP has fully retired the legacy `/api/v3/quote/A,B,C` comma endpoint (returns **403 "Legacy Endpoint … no longer supported"**). So the daily fast-path must use **parallel single `quote?symbol=` calls** (pool-of-5, ~505 calls ≈ <2 min at 300/min) OR simply fetch a short recent-history slice per stale ticker. The main win — parallel `historical-price-eod/full` backfill — is unaffected. Do NOT design 3.1 around batch-quote.
- **Stage 3.2:** use `analyst-estimates period=annual` (quarter is Premium-gated); `epsAvg` is the consensus EPS field; `numAnalystsEps` the analyst count; `date` is the fiscal-year-end (use it as the snapshot entry's `fyLabel` for the divergence fiscal-rollover guard the engine already implements).
- **Stage 3.4:** screener at default thresholds yields ~1,267 names — comfortably under the 2,000 hard cap; the effective universe (base ∪ screener) will be ~1,300–1,700.
- **Stage 3.5:** both `earnings-calendar` (full) and `insider-trading` are now available on FMP — decide FMP-vs-Finnhub ordering here (both full-coverage; pick one, document it).
