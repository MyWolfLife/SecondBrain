...
- **Dev server**: Python HTTP server on port 8080 via `.Codex/launch.json`
...
## Development Notes
- Codex is writing the entire app under user direction
...
**IMPORTANT: When the user types `/fixbishop {id}` or says "fix {id}", "please fix {id}", "do {id}", or any similar phrasing where `{id}` looks like a Firestore document ID (long alphanumeric string), follow the `/fixbishop` skill defined in `.Codex/skills/fixbishop/SKILL.md`.**
