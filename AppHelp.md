...
**Photos on an item:** Each itinerary or planning item can hold pictures — a trail map from AllTrails, a screenshot of a reservation, a photo of where you parked. Expand the item and use **📋 Paste** (copy an image, then click it) or **🖼️ Choose File**. There is no camera button here on purpose. You give each picture a **name** when you add it — it's pre-filled for you (the filename, or "Pasted image"), so you can just accept it. The item then lists the **names only**, and the picture itself is downloaded when you click a name; that's what keeps a photo-heavy trip fast to open. A **📷 3** badge on the collapsed row tells you how many it has. Clicking a name opens the picture full-screen with **Rename** and **Delete**. Item photos are kept sharper than plant photos so map text stays readable, and they show in **Travel mode** too — the trail map is the whole point at the trailhead. Deleting an item, a day, a planning group, or the project deletes its pictures along with it.
