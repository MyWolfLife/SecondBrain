...
**Build status**: Phases 1 and 2 are fully live, and Phase 3 (paid FMP) is underway. Phase 1 built the core — navigation, Universe manager, price data cache, detector engine, Backtest Lab, live Scan, candidate dossier, trade tickets, and the Scoreboard. Phase 2 added the Finnhub enrichment — quality + insider chips with a falling-knife flag on dips, post-earnings drift (Detector B), whole-market earnings chips with a ±typical-move risk gauge, and a dossier news feed with an optional AI emotional-vs-structural read. Phase 3 so far adds much faster price updates, the flagship price-vs-estimate **divergence** metric, and **revision momentum** (Detector C); still to come are market-wide screening and final consolidation. See `StockAnalyzerPlan.md` for the full design.
