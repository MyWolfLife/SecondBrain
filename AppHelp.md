...
- Every section header has a **?** icon -- tap it for a plain-language explanation of that section plus its detailed help
...
**Export:** **⬇️ Export** in the header downloads everything in this project as one `.json` file -- itinerary, bookings, locations, distances, to-dos, packing list, notes, and links -- so another person can **📥 Import** it into their own Bishop account, or so you have a backup. Only **people** are left out, since contacts are specific to your account and wouldn't mean anything on the other end. You're asked whether to **include photos** before it downloads -- each photo runs roughly 100–350KB, so a photo-heavy trip makes for a noticeably bigger file; leave it unchecked for a smaller, text-only export. **Receipts travel with the include-photos choice too** -- checked, every receipt (image + description + amount + category) comes along; unchecked, receipts are skipped entirely since a receipt without its image isn't much use. A progress screen shows what's being gathered, and tells you the final file size when it's done.

**Print / Save as PDF / Save as HTML:** **🖨️ Print** in the header opens a small choice -- **Print to PDF** or **Save as HTML** -- and the button beneath it reads **Print** or **Save** to match whichever you've picked. Both build the exact same paper-ready copy of the whole project; only what happens next differs:
- **Print to PDF** opens it in a new tab and immediately brings up your browser's print dialog -- choose **Save as PDF** there to get an actual PDF file, or print it for real. If your browser blocks the pop-up, it'll tell you to allow pop-ups for the site and try again.
- **Save as HTML** downloads it as one file instead -- everything is baked in (every photo, all the text), so you can email it or hand it to someone on a flash drive and they just double-click it to open the whole trip in their browser, no internet or Bishop account needed.

The document starts with a summary (dates, description, travelers, total cost with the same hotel/flight/activity breakdown you see on Trip Info, reference links), then the full **Itinerary** day by day -- each day gets a big bold heading of its own -- with every item expanded -- times, cost, confirmation, notes, **facts**, and **photos** shown one-per-row at full size so nothing is too small or cropped to read -- and, right under each item's location, its address, phone, and coordinates so you're not hunting for them separately -- **the address and the coordinates are each their own clickable link to Google Maps**, so if an address ever geocodes to the wrong spot (common for a trailhead or an odd parking area), tapping the coordinates instead takes you exactly where you meant. The full **Locations** reference at the back gets the same two clickable links per place. A short line divides items that aren't already separated by a travel summary. After that come **Bookings** (with screenshots), **Receipts** (each with its photo, description, category, and amount, plus a running total with the same per-category breakdown shown in the Receipts accordion), **Packing List**, **To-Do**, **Notes**, the **Photos** gallery, and finally a complete **Locations** and **Distances** reference at the very back. Anything you haven't filled in (a blank cost, empty notes, no photos, and so on) is simply left off rather than shown empty. **Every link -- project links, a booking's link, a location's website, a fact that's a URL -- is clickable, and shows the full address as its text**, so it works whether you're clicking it in the HTML file or just reading it off a printed page. The Planning Board and your People list aren't included -- Planning Board is exploratory, not-yet-decided content, and travelers are already named in the summary.

---

## screen:life-project-tripinfo

Think of Trip Info as the front cover of your trip. It shows when the trip starts and ends, a short description of it, and adds up all the money so you can see what the whole trip costs in one spot.
...
## screen:life-project-itinerary

Your itinerary is your day-by-day plan -- what you're doing on each day of the trip, in order. You make a card for each day and fill it with stops like flights, meals, and activities, and you can drag them around or move them to a different day.

---

**Itinerary:** The day-by-day plan. Auto-generate days from the project's date range (optionally adding pre/post travel days). **Changing a day's date** (tap ✏️ on the day, change the date, Save) asks whether to **shift all the other days by the same number of days** — say Yes and the whole itinerary slides forward or back together, with every day's label rebuilt to its new date; say No and only that one day moves. (Note: rebuilding a day's label replaces any custom text you typed into it.) Each **day card** is collapsible (click the header chevron); a collapsed day shows an item count. Drag days to reorder, and drag items within a day (drag handles). Add or edit an item through the shared modal -- title, status (confirmed/maybe/idea/nope), **type**, **start time**, duration, **leave by / arrival time**, cost, confirmation, contact, notes, **facts** (label+value rows; URL values become clickable links), a **location**, a **booking link**, a show-on-calendar toggle, and a **move-to** dropdown to send the item to another day or to the planning board. **Type** (None, Drive, Flight, Travel, Activity, Hotel) tailors the form: for **Flight**, **Activity**, and **Hotel** the Cost, Confirmation, and Contact fields appear; for **None, Drive, and Travel** they're hidden. The departure-time field is labeled **"Arrival Time"** for Drive/Flight/Travel, **"Check-in Time"** for Hotel, and **"Leave By"** for None/Activity. Pick **Activity** and a **Sub Type** dropdown appears — Other (the default), Hike, Sports Event, Tour, Viewpoint, Eat, Shopping, or Show. Choosing **Hike** or **Sports Event** also shows a checkbox — **"Trail Map Downloaded"** for a Hike, **"Tickets Downloaded"** for a Sports Event — so you can tick it off once you've grabbed what you need before the trip; it starts unchecked and stays hidden for the other sub types. Once set, the sub type and its checked status show in the item's expanded detail panel. Use **Hotel** to log where you're staying each night — it keeps a single location and shows the booking fields (cost, confirmation, contact). For the movement types (**Drive, Flight, Travel**) the location picker splits into **From Location** and **To Location** so you can capture both ends of the trip; for None/Activity there's just a single **Location**. Once you've picked a location, a **📍** and **✏️** appear next to that field — **📍** opens it in Google Maps, and **✏️** opens the address/phone/coordinates for that place right there so you don't have to leave the item to fix a typo or add a phone number; saving brings you back with the same location still selected. Delete an item with the **🗑️ Delete** button in the modal's action row (next to Save). If a day has journal entries within its date, a **📓** button appears on the day header and opens the Journal in a new tab for that date.
...
**Photos on an item:** Each itinerary or planning item can hold pictures — a trail map from AllTrails, a screenshot of a reservation, a photo of where you parked. There are two places to add one, and they show the same list: in the **edit form** (✏️, just below Facts), or in the item's **expanded detail panel** (click the item's title). Either way, use **📋 Paste** (copy an image, then click it) or **🖼️ Choose File**. There is no camera button here on purpose. Photos save the moment you add them — **Cancel** in the edit form will not undo a picture. You give each picture a **name** when you add it — it's pre-filled for you (the filename, or "Pasted image"), so you can just accept it. The item then lists the **names only**, and the picture itself is downloaded when you click a name; that's what keeps a photo-heavy trip fast to open. A **📷 3** badge on the collapsed row tells you how many it has. Clicking a name opens the picture full-screen with **Rename** and **Delete**. Item photos are kept sharper than plant photos so map text stays readable, and they show in **Travel mode** too — the trail map is the whole point at the trailhead. Deleting an item, a day, a planning group, or the project deletes its pictures along with it.

**Item rows:** Click an item's **title** to expand/collapse its detail panel. On desktop the collapsed row shows the status badge, title, booking/location badges, and edit/location buttons; on mobile the collapsed row shows just the title and moves the badges and buttons into the expanded panel. The **📍 location badge is a link** -- tap it to open that place in Google Maps (it uses the saved coordinates when it has them, so lakes and trailheads land in the right spot). For a **Drive, Flight, or Travel** item that has both a From and a To location set, the badges **stack — From on top of To** — and each one is its own clickable link to Maps. **Delete an item from inside its edit modal.**
...
**Photos:** Planning items hold pictures exactly like itinerary items do — which is usually where a trail map or a screenshot first lands, while the hike is still just an idea. Add one either from the **edit form** (✏️, below Facts) or from the item's **expanded detail panel** (click its title) — both show the same list. Use **📋 Paste** (copy an image anywhere, then click it) or **🖼️ Choose File**; there is no camera button. Pictures save as soon as you add them, so **Cancel** in the edit form will not take one back. Name it — the name is pre-filled, so you can just accept it. The item lists the **names**, and the picture downloads only when you click one, which is what keeps a board full of research quick to open. A **📷 3** badge on the collapsed row shows the count. Clicking a name opens the picture full-screen with **Rename** and **Delete**. Because photos are tied to the item itself, they **travel with it** — promote the hike to Day 5 and the trail map goes along, and again if you shuffle it to another day. Deleting the item, or its whole group, deletes its pictures too.

---

## screen:life-project-journal

This is a little diary for planning your trip. Write short notes to yourself -- ideas, reminders, or things you found -- and they're saved with the newest one on top.
...
## screen:life-project-todo

This is your trip checklist -- stuff to get done before you leave, like "renew passport" or "book the dog sitter." Check things off as you finish them, and drag the important ones to the top.

---

**To-Do** (planning mode only): A checklist with done toggles, notes, and drag-to-reorder. **Checked-off items drop to the bottom of the list** automatically, so what's left to do stays on top; unchecking an item brings it back up. Your drag order is preserved within each group. **Adding and editing happen right in the list** -- **+ Add Item** opens a blank row, and **✏️ Edit** turns an item into an inline editor with a field for the task and a box for its note/description, plus **Save** and **Cancel** (pressing Enter in the task field also saves). The summary shows done/total. The Vacation template pre-fills ~10 starter items.

---

## screen:life-project-photos

This is a photo album for pictures that help you plan -- screenshots, maps, or places you want to see. It's not for one specific hotel or flight; it's your trip's general picture box.
...
## screen:life-project-links

This is where you keep web links for your trip -- a video, a map, or a website you found. Click one to open it or copy it to share, so you don't have to hunt for that page again later.
...
## screen:life-project-bookings

Bookings is where you keep all your reservations -- flights, hotels, tours, anything you booked. Each one holds the important details like dates, price, and the confirmation number, so it's all in one place instead of buried in your email.
...
This is a running list of receipts from the trip, plus what they cost. Snap a photo of a receipt (or paste one you copied), then jot a short note like "Dinner Wed night," the date, how much it was, and what kind of expense it was -- the list sorts newest-first by that date, and the section header keeps a running total so you can see what you've spent without adding it all up yourself. Traveling somewhere that doesn't use US Dollars? Tap the ⚙️ next to the section title to set up a country and its exchange rate -- then when you log a receipt you just type the local amount and the app shows you the US Dollar equivalent, no napkin math required.

---

**Receipts:** A running log of receipts -- with or without a photo -- each with a description, date, amount, and category. Add via **🖼️ Add Image** (file picker, supports multi-select), **📷 Camera** (capture), **📋 Paste** (clipboard), or **📝 No Photo** (for a receipt with nothing to photograph, e.g. a cash tip) -- after the image is added (or right away for No Photo), a **Receipt Details** modal asks for a Description (e.g. "Dinner Wed night"), a Date (defaults to today, editable), a **Currency** dropdown (only appears once at least one currency is set up -- see below), an Amount, and a Category (dropdown: Meals, Snacks, Parking, Souvenirs, Alcohol, Gas, Activity, Other). Each receipt shows as a row with its description, date, category, and amount, sorted by date descending (newest first) -- a non-USD receipt also shows its original local amount in parentheses, e.g. `$73.00 (100.00 Canada)`, though the USD figure is always what's used in the running total; if it has a photo, tapping the description opens a **lightbox** with the full image, description, date + category + amount, an ✏️ Edit button (reopens the same Details modal, pre-filled with the receipt's existing date and currency), 🗑️ Delete, and ✕ Close -- a No Photo receipt's description is plain text instead (no lightbox), but still has its own ✏️ Edit and 🗑️ Delete buttons on the row. Editing and changing the date re-sorts the list. The **accordion header** shows a running total followed by a per-category breakdown in parentheses, e.g. `$142.50 (Meals: $80.00, Parking: $20.00, Souvenirs: $30.00, Snacks: $12.50)` -- same style as the Trip Info cost breakdown, always in USD. Stored in the `receipts` subcollection (Base64 image, blank for No Photo entries + description + date + amount [USD] + category + currencyName + localAmount + rate + createdAt). Visible in both planning and travel modes.

**Setup ⚙️ (Receipt Currencies):** Tap the ⚙️ gear next to the Receipts section title to manage the countries/currencies you pay in while traveling. Each entry has a name (e.g. "Canada") and a conversion rate meaning "1 [name] = ___ USD" (leave blank for 1:1). Use ▲▼ to reorder -- the **top entry is the default** offered when you next add a receipt, so move whichever country you're currently in to the top before logging receipts there. US Dollar is always available automatically and always appears last in the Currency dropdown; you don't add it yourself. If you haven't set up any currencies, the Currency dropdown doesn't show at all and every receipt is plain USD, same as before this feature existed. When you pick a non-USD currency on a receipt, the Amount field becomes "Amount ({currency})" and a live "≈ US Dollar" field appears below it, auto-calculated as you type -- but you can still edit that USD figure yourself if the real conversion was slightly different (card fees, etc.). Deleting a currency doesn't change receipts you already logged in it -- they keep showing their original amount and currency name, you just can't pick that currency again for new receipts.

---

## screen:life-project-packing

This is your packing list so you don't forget anything. Your stuff is sorted into groups like clothes and toiletries, and you check each thing off as it goes in the bag.
...
## screen:life-project-locations

This is your list of real places for the trip -- hotels, restaurants, parks, whatever. Once a place is on the list you can use it on your daily plan, and the app can even figure out how far apart two places are.
...
## screen:life-project-distances

This shows how long it takes to get from one of your places to another -- like 20 minutes by car. You add these so your daily plan knows travel times and can tell you when you'll arrive; it's also what makes the "travel time needed" warning go away.
...
## screen:life-project-people

This is the list of people going on the trip or helping with it. You can pick them from your contacts and add a note about each one.
...
- **🗂 Were your dismissals right?** breaks your dismissals down by the reason you gave — a dismissal counts as "right" when the stock then missed +10% (you correctly skipped a dud). So you find out whether your "falling knife" calls actually save you, or whether "don't like the story" is costing you winners
- **🤖 Is the AI read any good?** grades the AI's own calls: an EMOTIONAL/SUPPORTED read is right when the stock hit +10%, a STRUCTURAL/CONTRADICTED read when it missed. The AI earns trust with a track record like everything else — until then, both breakdowns show a quiet "N of 5 so far" while they accrue
- Each scan's table has an **AI read** column showing the recorded verdict (hover for the full line), so you can see what the AI said next to what actually happened
...
- **Editing tags** (in edit mode): each tag chip in the footer gets a ✕ to remove it, and a **"+ tag"** box appears — type a tag and press Enter to add it (it suggests tags you've used elsewhere as you type). Tag changes affect only this list, not the template it came from.
...
- **Collapse a list**: tap the **▾ chevron** at the top of a card to collapse it down to just its title row, with the item count shown next to the name (tap **▸** to expand again). Handy for tucking away long lists you're not working on right now. Collapsed/expanded state is remembered per device (it doesn't sync between your phone and computer).
- **Filter by tag**: a **Filter by Tag** panel lists every tag on your active lists as a checkbox — on the computer it sits to the left of the board; on the phone it's a tap-to-open panel above the board. Check one or more tags to narrow the board; checking several **combines them** (a list must have *all* the checked tags to show). It works together with the text filter, and a **Clear tags** link resets it. Note: while any filter (text or tag) is on, list reordering is paused.
...
- **Summary Card** (collapsible, at the top): tap the header to expand a per-activity-type breakdown for the selected month — one row per type showing **Times** (count), **Total Time**, **Burned** (calories), **Weight** (pounds-equivalent of the burn — calories ÷ 3500), **Cal/Hour** (calories burned per hour), and distance columns. Weight and Cal/Hour are blank when a row has no calories (and Cal/Hour is also blank with no duration). For run/walk types it splits distance into **Walked**, **Ran**, and **Total**; other distance types show a single **Distance** value (meters types like Row Machine show "m"). A bold **Total** row at the bottom sums everything — except **Weight** and **Cal/Hour**, which are recalculated from the totals (total burn ÷ 3500, and total burn ÷ total hours) rather than added up. Its **Total** cell is the combined miles of the whole Walked + Ran columns (all your walking and running for the month), even though the per-row Total stays blank for walk-only or run-only activities. (Distance is left blank in the total row since types use mixed units.) Starts collapsed each time you open the screen; always covers the whole month regardless of the activity-type filter. On phones the table scrolls sideways within the card.
...
- **📊 Summary accordion** (single-month view only, above the filter): tap the header to expand a month-at-a-glance weight-loss picture plus your activities grid. Your open/closed choice is remembered between visits (starts collapsed).
  - **Top numbers** — the calorie inputs (**Burned** = your total burn for the month, **Exercise** = calories from logged workouts, **Eaten** = food calories) followed by four pounds figures. **▼ green** means pounds lost, **▲ red** means gained:
    ...
    - **Actual so far** — what the scale actually shows this month (oldest weigh-in vs. the ending weigh-in). For a **completed past month** the ending weigh-in is the 1st of the next month, so it matches the Totals row and the Ending Weight row in the list.
...
- **⚖ Weight Chart accordion**: a chart of your weight over time — tap the header to expand or collapse. Your choice is remembered between visits. Use the range dropdown inside to pick Last 7 Days, Last 2 Weeks, Last 30 Days, **Selected Month**, Last 90 Days, This Year, or All Time. The chart always holds a sensible minimum width (as if there were at least 7 days of data), so early-month or sparse views don't render as a razor-thin sliver. Next to the range dropdown is a **⚙ Settings** button that opens a small popup with four options:
  - **Rolling Average Days**: how many days the smoothing (average) line covers. The default is 3 (a 3-day average); change it to 5, 10, or anything from 2 to 60. The line's label in the key updates to match (e.g. "10-Day Avg"). This is remembered between visits, and the Daily Metrics chart and the Summary chart each keep their own value.
  - **Show Projected Weight**: adds a **light-blue dotted line** showing what your weight would be each day if it were driven purely by your calorie balance — it starts at your first weigh-in **on or after the first day you logged food calories** (so it doesn't draw a useless flat line back before you had any calorie data), and from there each day adds or subtracts that day's calorie loss/gain (burn − food) at 3,500 calories per pound. Days with no burn/food logged count as no change (the line carries flat across them). Comparing this line to your actual weight shows how closely your real weight is tracking what the calories predict. Off by default; remembered between visits.
  - **Show Missing Days**: by default the chart only puts a slot on the x-axis for days you actually weighed in, so gaps are skipped. Turn this on to show **every calendar day** between your first and last weigh-in along the bottom — days with no weigh-in still appear on the axis but get **no dot**. All the lines simply bridge across the empty days, so the Goal line still passes straight through them and the average line is unchanged (it just lines up with real date spacing). Off by default; remembered between visits.
  - **Reduce Width %**: if a range still looks stretched, type a percent here to tighten the chart — each entry shrinks it a bit more (type 10 to drop to 90%, type 10 again for 81%), and the box clears to 0 after each one. Changing the date range puts it back to full width. (Since the popup covers the chart, close it to see the new width.)
  
  A **key (legend)** above the chart labels each line. The chart shows up to four lines: a **blue line** (your actual weight — click any dot to open that day's entry), an **orange line** (the rolling average, using your Rolling Average Days setting, to smooth out noise), the **light-blue dotted Projected line** (when Show Projected Weight is on), and a **dashed green line** (Goal — only visible when "Selected Month" is chosen and you have a goal weight set for that month in your Exercise Goals). The goal line runs from your first weigh-in of the month down to your month-end goal weight, so you can see at a glance whether your actual weight is tracking above or below plan. **Y-axis labels appear on both left and right sides** so the scale is always readable even at the far right of the chart. **Past months in "Selected Month" view** include the first weigh-in of the following month as a final data point — for example, the May chart includes June 1st's weight — because that reading is really the ending weight for May.
...
- **Ending Weight row**: when viewing a completed past month, the list shows the first weigh-in of the **following** month at the top — a dimmed row on desktop (with an "ending wt" tag) or a dimmed card on mobile, both labeled **Ending Weight**. This is the true ending weight of the month (e.g. July shows Aug 1's weight), the same point the chart plots as the month's final dot. The **Totals** summary and the **Summary accordion's "Actual so far"** both use this ending weight to compute the month's final gain/loss, so all three reconcile; the **Averages** summary divides by the full number of days in the month (not just the span between weigh-ins). You can tap it to open that day's entry.
...
- **⚖ Weight Chart accordion**: a chart of your weight for the whole year — tap to expand. It defaults to the **Selected Year** range (year-to-date for the current year, the full year for a past year); you can also switch to Last 30/90 Days, This Year, or All Time. A **⚙ Settings** button opens the same popup as Daily Metrics — Rolling Average Days, Show Projected Weight, Show Missing Days, and Reduce Width %. Its Rolling Average Days value is kept separately from the Daily Metrics chart's, so you can smooth this year-long view differently.
