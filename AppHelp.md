...
**Receipts:** A running log of receipt photos with a description, date, amount, and category each. Add via **🖼️ Add Image** (file picker, supports multi-select), **📷 Camera** (capture), or **📋 Paste** (clipboard) -- after the image is added, a **Receipt Details** modal asks for a Description (e.g. "Dinner Wed night"), a Date (defaults to today, editable), an Amount, and a Category (dropdown: Meals, Snacks, Parking, Souvenirs, Alcohol, Gas, Activity, Other). Each receipt shows as a row with its description, date, category, and amount, sorted by date descending (newest first); tapping the description opens a **lightbox** with the full image, description, date + category + amount, an ✏️ Edit button (reopens the same Details modal, pre-filled with the receipt's existing date), 🗑️ Delete, and ✕ Close. Editing and changing the date re-sorts the list. The **accordion header** shows a running total followed by a per-category breakdown in parentheses, e.g. `$142.50 (Meals: $80.00, Parking: $20.00, Souvenirs: $30.00, Snacks: $12.50)` -- same style as the Trip Info cost breakdown. Stored in the `receipts` subcollection (Base64 image + description + date + amount + category + createdAt). Visible in both planning and travel modes.
