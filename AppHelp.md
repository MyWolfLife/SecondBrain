...
**Receipts:** A running log of receipts -- with or without a photo -- each with a description, date, amount, and category. Add via **🖼️ Add Image** (file picker, supports multi-select), **📷 Camera** (capture), **📋 Paste** (clipboard), or **📝 No Photo** (for a receipt with nothing to photograph, e.g. a cash tip) -- after the image is added (or right away for No Photo), a **Receipt Details** modal asks for a Description (e.g. "Dinner Wed night"), a Date (defaults to today, editable), an Amount, and a Category (dropdown: Meals, Snacks, Parking, Souvenirs, Alcohol, Gas, Activity, Other). Each receipt shows as a row with its description, date, category, and amount, sorted by date descending (newest first); if it has a photo, tapping the description opens a **lightbox** with the full image, description, date + category + amount, an ✏️ Edit button (reopens the same Details modal, pre-filled with the receipt's existing date), 🗑️ Delete, and ✕ Close -- a No Photo receipt's description is plain text instead (no lightbox), but still has its own ✏️ Edit and 🗑️ Delete buttons on the row. Editing and changing the date re-sorts the list. The **accordion header** shows a running total followed by a per-category breakdown in parentheses, e.g. `$142.50 (Meals: $80.00, Parking: $20.00, Souvenirs: $30.00, Snacks: $12.50)` -- same style as the Trip Info cost breakdown. Stored in the `receipts` subcollection (Base64 image, blank for No Photo entries + description + date + amount + category + createdAt). Visible in both planning and travel modes.
