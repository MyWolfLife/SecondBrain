...
| `PwaPlan.md` | App-wide | PWA implementation — installable icon, offline support, service worker, custom Firebase config. **Phases 1, 2 & 4 COMPLETE.** Phase 2.5 (Go Offline/Go Online) core mechanism BUILT; the read-only lock enforcement part is not yet built (needs its own scoping pass — see doc). Phase 3 (install-prompt polish) and Phase 5 (push notifications) not started. |
